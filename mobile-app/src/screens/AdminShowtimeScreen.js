    ...
    const handleDelete = async (id) => {
        const executeDelete = async () => {
            try {
                ...
                const errorMsg = err.response?.data?.error || 'Failed to delete showtime';
                if (Platform.OS === 'web') {
        ...
        return Object.values(grouped).map((group, index) => (
            <View key={index} style={styles.movieGroupCard}>
                <View style={styles.movieGroupHeader}>
                    {group.poster ? (
                        <Image 
                            source={{ uri: `${BASE_URL}/uploads/movies/${group.poster}` }} 
...
const styles = StyleSheet.create({
    ...
    searchContainer: { paddingHorizontal: 20, marginTop: 25, marginBottom: 15 },
    searchBar: { flexDirection: 'row', backgroundColor: '#161B2E', padding: 12, borderRadius: 18, alignItems: 'center', borderWidth: 1, borderColor: '#1F2937' },
    searchInput: { flex: 1, marginLeft: 10, color: '#fff', fontSize: 16 },
    listContent: { padding: 20, paddingTop: 5 },
    movieGroupCard: { backgroundColor: '#161B2E', borderRadius: 28, marginBottom: 24, overflow: 'hidden', borderWidth: 1, borderColor: '#1F2937' },
    movieGroupHeader: { flexDirection: 'row', padding: 16, backgroundColor: 'rgba(255,255,255,0.02)', alignItems: 'center', borderBottomWidth: 1, borderBottomColor: '#1F2937' },
    groupPoster: { width: 50, height: 75, borderRadius: 12 },
    groupTitle: { fontSize: 18, fontWeight: 'bold', color: '#fff', marginBottom: 4 },
    schedulesContainer: { padding: 16 },
    scheduleRow: { flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 14, borderBottomWidth: 1, borderBottomColor: '#1F2937' },
    scheduleDate: { color: '#fff', fontSize: 15, fontWeight: 'bold', marginLeft: 8 },
    timeBadge: { backgroundColor: '#1F2937', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 8, marginRight: 6, marginBottom: 6 },
